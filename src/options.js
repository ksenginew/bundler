...
export async function normalizeInputOptions(config, watchMode) {
    ...
    const normalizePluginOption = async (/** @type {any} */ plugins) => (await asyncFlatten([plugins])).filter(Boolean);
    /**
...
const getAcornInjectPlugins = (/** @type {import("./types").InputOptions} */ config) => [
    importAssertions,
    ...(Array.isArray(config.acornInjectPlugins) ? config.acornInjectPlugins : [config.acornInjectPlugins])
]
